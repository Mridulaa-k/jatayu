...
client.loop_start()  # Ensures MQTT messages are sent properly
...
alert_triggered = False  # Prevents multiple alerts for the same sleep event
...
# Small delay to ensure camera starts properly
time.sleep(2)
...
while True:
    ...
    for face in faces:
        ...
        if is_sleeping:
            ...
            alert_triggered = False  # Reset alert flag when eyes are open

        # Trigger sleep alert only if eyes are closed for FRAME_THRESHOLD frames
        if sleep_frame_count >= FRAME_THRESHOLD and not alert_triggered:
            cv2.putText(frame, "Person is Sleeping!", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
            client.publish(MQTT_TOPIC, "ALERT")
            print("MQTT ALERT SENT!")  # Debug print
            alert_triggered = True  # Prevent continuous alerts
